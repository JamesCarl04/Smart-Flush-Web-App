...
import type { TaskApiData, TaskDoc } from '@/lib/task-types';
...
function taskNotificationBody(task: Pick<
  TaskApiData | TaskDoc,
  'component' | 'location' | 'floor' | 'building' | 'message'
>): string {
  if (task.component && task.location && task.floor && task.building) {
...
export async function sendTimeoutWarningNotification(
  ...
  await sendAppNotification(supervisorUid, {
    title: 'Task Not Acknowledged',
    body: `${assigneeName} has not acknowledged the task for 5 minutes`,
    data: {
