          ...
          <div className="flex-none">
            <div className="dropdown dropdown-end">
              <div
                ...
                data-tip={`${unreadCount} unread alerts`}
              >
                <div
                  tabIndex={0}
                  role="button"
              ...
              <ul
                tabIndex={0}
                className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-200 rounded-box w-64 border border-base-300"
              >
                <li className="menu-title px-4 py-2 text-xs font-bold uppercase tracking-wider text-base-content/60">
                ...
                {recentAlerts.map((alert) => (
                  <li key={alert.id}>
                    <Link
                      href="/alerts"
                      className={`flex flex-col items-start gap-1 py-3 ${!alert.acknowledged ? 'bg-base-300/50 font-medium' : ''}`}
                    >
        ...
        <div className="navbar sticky top-0 z-40 hidden w-full border-b border-base-200 bg-base-100/50 px-8 backdrop-blur-md transition-colors duration-300 lg:flex">
          ...
          <div className="flex-none gap-4">
            ...
            <div className="dropdown dropdown-end">
              ...
              <ul
                tabIndex={0}
                className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-xl bg-base-100 rounded-box w-80 border border-base-200"
              >
                <li className="menu-title px-4 py-2 text-xs font-bold uppercase tracking-wider text-base-content/60 border-b border-base-200 mb-2">
                  Alerts ({unreadCount} unread)
                </li>
                {recentAlerts.length === 0 ? (
                  <li className="p-4 text-center text-base-content/50 pointer-events-none">
                    No recent alerts
                  </li>
                ) : (
                  recentAlerts.map((alert) => (
                    <li key={alert.id}>
                      <Link
                        href="/alerts"
                        className={`flex flex-col items-start gap-1 py-3 ${!alert.acknowledged ? 'bg-base-200/50' : ''}`}
                      >
                        <div className="flex items-start justify-between w-full">
                          <span
                            className={`truncate w-full block ${!alert.acknowledged ? 'font-bold' : ''}`}
                          >
                            {alert.title}
                          </span>
                          {!alert.acknowledged && (
                            <div className="w-2 h-2 rounded-full bg-error shrink-0 mt-1.5 ml-2"></div>
                          )}
                        </div>
                        <span className="text-xs text-base-content/50">
                          {formatDistanceToNow(new Date(alert.timestamp), {
