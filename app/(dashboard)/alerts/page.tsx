...
import { Bell, CheckCircle2, AlertTriangle, Info, AlertOctagon, CheckSquare, AlertCircle } from "lucide-react";
...
export default function AlertsPage() {
  ...
  const handleAcknowledge = async (id: string | "ALL") => {
    const idsToDismiss =
      id === "ALL"
        ? filteredAlerts.filter((alert) => !alert.acknowledged).map((alert) => alert.id)
        : [id];

    setDismissingIds((current) => Array.from(new Set([...current, ...idsToDismiss])));
    ...
    setDismissingIds((current) => current.filter((dismissedId) => !idsToDismiss.includes(dismissedId)));
  };
    ...
    <div className="container mx-auto max-w-5xl animate-fade-in p-4 md:p-8">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 text-base-content/60">Monitor critical hardware deviations and system notifications.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="btn btn-neutral btn-sm" onClick={() => handleAcknowledge("ALL")} disabled={unreadCount === 0 || loading}>
      ...
      <div className="card border border-base-200 bg-base-100 shadow-xl">
        <div className="card-body p-0">
          <div className="tabs tabs-bordered border-b border-base-200 p-4">
            <button className={`tab tab-lg ${filter === "all" ? "tab-active font-bold" : ""}`} onClick={() => setFilter("all")}>
              All Alerts
            </button>
            <button
            ...
            <button className={`tab tab-lg ${filter === "unread" ? "tab-active font-bold" : ""}`} onClick={() => setFilter("unread")}>
              Unacknowledged
            </button>
          </div>
          ...
          <div className="bg-base-100/50 p-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center text-base-content/50">
                ...
                <h3 className="mb-2 text-xl font-semibold">You&apos;re all caught up!</h3>
                    ...
                    >
                      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                        <div className="mt-1 flex items-start gap-4">
                          <div className="mt-1 shrink-0">{severityMeta.rowIcon}</div>
                          <div>
                            <div className="mb-1 flex items-center gap-3">
                              <h3 className={`font-bold ${alert.acknowledged ? "text-base-content/70" : ""}`}>{alert.title}</h3>
                              <div className={`badge badge-sm gap-1 border uppercase tracking-wider text-[10px] ${severityMeta.badgeClassName}`}>
                                {severityMeta.icon}
                                {alert.severity}
                              </div>
                            </div>
                            <p className="w-full text-sm text-base-content/70 md:max-w-xl">{alert.description}</p>
                        ...
                        <div className="flex w-full shrink-0 justify-end md:w-auto">
                          {!alert.acknowledged ? (
                            <button className="btn btn-outline btn-primary btn-sm w-full md:w-auto" onClick={() => handleAcknowledge(alert.id)}>
